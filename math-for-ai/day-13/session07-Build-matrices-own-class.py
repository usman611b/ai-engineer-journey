...
for i, u in enumerate(basis): #enumerate() is a built-in Python function that adds a counter to an iterable and returns it as an enumerate object. 
    #In this case, it is used to iterate over the orthonormal basis vectors generated by the Gram-Schmidt process. 
    # The variable i represents the index of the current vector in the basis list, and u represents the current orthonormal vector itself. 
    # 
    # This allows for easy access to both the index and the vector during iteration.
 print(f"u{i+1} = {u}")
